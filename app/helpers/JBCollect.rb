...
#require 'Hpricot'
    ...
    @date = Date.strptime(@date, fmt='%m/%d/%Y')
		#Create output array of each flight record
		  ...
		  for i in (0..j)
					Flight.create(:airline => "JB", :observationDate => Time.now.ctime, :flightDate => @date, :origin => @fromCity, :destination => @toCity, :price => price_array[i], :flightNumber => flighta_array[i], :departs => cleaned_departs_array[i], :arrives => cleaned_arrives_array[i], :seats => cleaned_seats_array[i])
 		  end
	end
end